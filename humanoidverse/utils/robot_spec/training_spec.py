...
class RobotTrainingSpec:
    ...
    def to_env_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "action_scale_by_joint": dict(self.action_scale_by_joint),
...
def load_robot_training_spec(config_path: str | Path) -> RobotTrainingSpec:
    ...
    action_scale_by_joint_raw = control.get("action_scale_by_joint", {})
    if not isinstance(action_scale_by_joint_raw, dict):
        raise ValueError("training.control.action_scale_by_joint must be a mapping")
    unknown_action_scale_joints = sorted(set(action_scale_by_joint_raw) - set(control_joints))
    if unknown_action_scale_joints:
        raise ValueError(
            "training.control.action_scale_by_joint contains unknown joints: "
            f"{unknown_action_scale_joints}"
        )
    action_scale_by_joint = {str(joint): float(value) for joint, value in action_scale_by_joint_raw.items()}
    ...
    if actuator_source == "yaml":
        ...
        if profiles_raw and not isinstance(profiles_raw, dict):
            raise ValueError("training.actuator.profiles must be a mapping when provided")
        profiles = {str(name): profile for name, profile in profiles_raw.items()} if isinstance(profiles_raw, dict) else {}
        for profile_name, profile in profiles.items():
            if not isinstance(profile, dict):
                raise ValueError(f"training.actuator.profiles.{profile_name} must be a mapping")
            for key in (
                "armature",
                "stiffness",
                "damping",
                ...
                if key in profile:
                    float(profile[key])
            for key in ("delay_min_lag", "delay_max_lag"):
                if key in profile:
                    int(profile[key])
        for joint in control_joints:
            ...
            if profile_name is not None and str(profile_name) not in profiles:
                raise ValueError(
                    f"training.actuator.joints.{joint}.profile={profile_name!r} "
                    "does not exist in training.actuator.profiles"
                )
    ...
    return RobotTrainingSpec(
        ...
        action_scale_by_joint=action_scale_by_joint,
