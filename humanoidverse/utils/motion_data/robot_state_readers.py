...
def _required_pkl_array(record: dict[str, Any], path: Path, field: str, width: int | None = None) -> np.ndarray:
    if field not in record:
        raise ValueError(f"robot_state_pkl source file={path} missing required field {field!r}")
    array = np.asarray(record[field], dtype=np.float32)
    if width is not None and (array.ndim != 2 or array.shape[1] != width):
        raise ValueError(f"robot_state_pkl source file={path} field {field!r} must have shape [T, {width}], got {array.shape}")
    return array


def _fps_from_pkl(record: dict[str, Any], path: Path, fps: float | int | None) -> float:
    if fps is not None:
        motion_fps = float(fps)
    elif "fps" in record:
        motion_fps = float(_scalar_from_npz(record["fps"]))
    elif "framerate" in record:
        motion_fps = float(_scalar_from_npz(record["framerate"]))
    else:
        raise ValueError(f"robot_state_pkl source file={path} requires fps, framerate, or manifest fps")
    if not np.isfinite(motion_fps) or motion_fps <= 0.0:
        raise ValueError(f"robot_state_pkl source file={path} has invalid fps={motion_fps}")
    return motion_fps


def _pkl_root_quat(record: dict[str, Any], path: Path, quat_order: str) -> np.ndarray:
    if "root_quat" in record:
        root_quat = _required_pkl_array(record, path, "root_quat", 4)
    elif "base_quat_w" in record:
        root_quat = _required_pkl_array(record, path, "base_quat_w", 4)
    else:
        raise ValueError(f"robot_state_pkl source file={path} missing root_quat/base_quat_w")
    if quat_order == "xyzw":
        return root_quat
    if quat_order == "wxyz":
        return root_quat[:, [1, 2, 3, 0]].copy()
    raise ValueError(f"robot_state_pkl source file={path} has unsupported root_quat_order={quat_order!r}")


def _pkl_joint_names(record: dict[str, Any], path: Path) -> list[str] | None:
    if "joint_names" not in record:
        logger.warning(
            f"robot_state_pkl file={path} has no joint_names; assuming dof_pos is ordered as RobotSpec.control_joint_names"
        )
        return None
    return _string_list_from_npz(record["joint_names"])
...
def read_robot_state_pkl(
    ...
    *,
    ...
    config = dict(columns or {})
    root_pos_field = str(config.get("root_pos", "base_pos_w"))
    dof_pos_field = str(config.get("dof_pos", "joint_pos"))
    ...
    for path in expand_motion_paths(path_spec, base_dir=base_dir, suffix=".pkl"):
        ...
        if not isinstance(record, dict):
            raise ValueError(f"robot_state_pkl source file={path} must contain a mapping, got {type(record).__name__}")
        motion_key = str(record.get("motion_key", path.stem))
        if motion_key in data:
            raise ValueError(f"Duplicate motion_key={motion_key} while reading robot_state_pkl source={source_name}")
        data[motion_key] = RobotStateMotion(
            motion_key=motion_key,
            root_pos=_required_pkl_array(record, path, root_pos_field, 3),
            root_quat=_pkl_root_quat(record, path, root_quat_order),
            dof_pos=_required_pkl_array(record, path, dof_pos_field, len(robot_spec.control_joint_names)),
            fps=_fps_from_pkl(record, path, fps),
            joint_names=_pkl_joint_names(record, path),
            source=source_name,
            metadata={
                "path": str(path),
