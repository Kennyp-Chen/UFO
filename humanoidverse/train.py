...
def build_ufo_mjlab_config(
    *,
    ...
    selected = build_agent_preset(
        agent=agent,
        device=agent_device,
        compile=(not distributed_sync) or enable_compile,
...
def _resolve_initial_model_path(path: str | Path) -> Path:
    candidate = Path(path).expanduser().resolve()
    if candidate.is_file():
        if candidate.name != "model.safetensors":
            raise ValueError(f"--init-model must point to model.safetensors, got: {candidate}")
        return candidate
    if not candidate.is_dir():
        raise FileNotFoundError(f"Initial model path does not exist: {candidate}")
    for model_path in (candidate / "model.safetensors", candidate / "model" / "model.safetensors", candidate / "checkpoint" / "model" / "model.safetensors"):
        if model_path.is_file():
            return model_path
    raise FileNotFoundError(f"Could not find model.safetensors below initial model path: {candidate}")


def _initialize_agent_model(workspace, init_model: str | Path, device: str) -> Path:
    from safetensors.torch import load_model

    model_path = _resolve_initial_model_path(init_model)
    missing, unexpected = load_model(workspace.agent._model, str(model_path), strict=True, device=device)
    if missing or unexpected:
        raise RuntimeError(
            f"Initial model is incompatible with the current training configuration: missing={missing}, unexpected={unexpected}"
        )
    workspace.agent._model.train()
    workspace.agent._model.requires_grad_(True)
    return model_path
...
def run_train(args: argparse.Namespace, log_dir: Path) -> None:
    ...
    if args.init_model is not None and (log_dir / "checkpoint" / "train_status.json").exists():
        raise ValueError("--init-model requires a fresh --work-dir without checkpoint/train_status.json")
    print(
        "[INFO] UFO train: "
    ...
    try:
        ...
        if args.init_model is not None:
            model_path = _initialize_agent_model(workspace, args.init_model, device)
            print(f"[INFO] Initialized agent model from {model_path}; optimizer and replay buffer start fresh.", flush=True)
        workspace.train()
    finally:
        if world_size > 1:
            import torch.distributed as dist
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable-compile",
        action="store_true",
        help="Enable torch.compile for distributed training. Disabled by default because first-run compilation is expensive and backend support is hardware-dependent.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Initialize a fresh training run from model.safetensors (or its model/checkpoint/run directory). "
            "This loads model weights only; optimizer and replay state start fresh."
        ),
    )
    parser.add_argument(
