...
def _g1_mjlab_mode15_actuator_params(dof_names: tp.Sequence[str]) -> dict[str, list[float]]:
    ...
    return {
        ...
        "armature": armatures,
        "friction": frictions,
        # Keep the built-in G1 profile compatible with the generic actuator
        # path used by YAML-defined robots. Mode-15 has no extra motor model
        # metadata, so its physical limits are the appropriate defaults.
        "model": ["dc_motor"] * len(dof_names),
        "saturation_effort": list(efforts),
        "motor_velocity_limit": list(velocities),
        "curve_param_a": [0.0] * len(dof_names),
        "curve_param_b": [0.0] * len(dof_names),
        "curve_param_c": [0.0] * len(dof_names),
        "max_torque": list(efforts),
        "max_velocity": list(velocities),
        "delay_min_lag": [0] * len(dof_names),
        "delay_max_lag": [0] * len(dof_names),
        "profile": ["g1_mode15"] * len(dof_names),
    }
...
def _patch_humanoidverse_robot_config(config, robot_training: dict[str, Any] | None) -> None:
    ...
    config.robot.control.action_scale_by_joint = dict(robot_training.get("action_scale_by_joint") or {})
...
def _actuator_params_from_training(dof_names: tp.Sequence[str], robot_training: dict[str, Any] | None) -> tuple[str, dict[str, list[float]]]:
    ...
    profiles_raw = actuator.get("profiles") or {}
    profiles = {str(name): dict(profile) for name, profile in profiles_raw.items()} if isinstance(profiles_raw, dict) else {}
    for joint_name in dof_names:
        ...
        profile_name = str(joint_params.get("profile", ""))
        profile = profiles.get(profile_name, {})
        # Keep legacy YAML actuator configs on DcMotorActuatorCfg unless a
        # profile explicitly selects another model.
        params["model"].append(str(profile.get("model", "dc_motor")))
        params["profile"].append(profile_name or "default")
        params["saturation_effort"].append(float(profile.get("saturation_effort", joint_params["effort_limit"])))
        params["motor_velocity_limit"].append(
            float(profile.get("motor_velocity_limit", profile.get("max_velocity", joint_params["velocity_limit"])))
        )
        params["curve_param_a"].append(float(profile.get("curve_param_a", 0.0)))
        params["curve_param_b"].append(float(profile.get("curve_param_b", 0.0)))
        params["curve_param_c"].append(float(profile.get("curve_param_c", 0.0)))
        params["max_torque"].append(float(profile.get("max_torque", profile.get("saturation_effort", joint_params["effort_limit"]))))
        params["max_velocity"].append(float(profile.get("max_velocity", profile.get("motor_velocity_limit", joint_params["velocity_limit"]))))
        params["delay_min_lag"].append(int(profile.get("delay_min_lag", 0)))
        params["delay_max_lag"].append(int(profile.get("delay_max_lag", 0)))
...
def _action_target_scale(config) -> torch.Tensor:
    ...
    action_scale_by_joint = dict(config.robot.control.get("action_scale_by_joint") or {})
    scales = []
    for i, joint_name in enumerate(dof_names):
        kp = _match_joint_value(joint_name, stiffness)
        scale = float(action_scale_by_joint.get(joint_name, config.robot.control.action_scale))
...
def make_mjlab_ufo_env_cfg(
    ...
    *,
    ...
    from humanoidverse.agents.envs.ht_motor_mjlab import HTMotorActuatorCfg
    ...
    def spec_fn():
        ...
        # The robot XMLs are also used by standalone MuJoCo renderers and may
        # contain their own visual floor. MJLab imports a terrain entity below,
        # so keep exactly one plane in the composed simulation.
    ...
    saturation_efforts = actuator_params["saturation_effort"]
    motor_velocity_limits = actuator_params["motor_velocity_limit"]
    delay_min_lags = actuator_params["delay_min_lag"]
    delay_max_lags = actuator_params["delay_max_lag"]
    ...
    for i, joint_name in enumerate(dof_names):
        ...
        profile_model = actuator_params["model"][i]
        common = dict(
            target_names_expr=(joint_name,),
            stiffness=kp,
            damping=kd,
            delay_min_lag=delay_min_lags[i],
            delay_max_lag=delay_max_lags[i],
            armature=armature[i] if i < len(armature) else None,
            frictionloss=friction[i] if i < len(friction) else None,
        )
        if actuator_source == "yaml" and profile_model == "ht_motor":
            actuators.append(
                HTMotorActuatorCfg(
                    **common,
                    effort_limit=actuator_params["max_torque"][i],
                    curve_param_a=actuator_params["curve_param_a"][i],
                    curve_param_b=actuator_params["curve_param_b"][i],
                    curve_param_c=actuator_params["curve_param_c"][i],
                    max_torque=actuator_params["max_torque"][i],
                    max_velocity=actuator_params["max_velocity"][i],
                )
            )
            actuator_types.append("HTMotor")
        elif actuator_source == "yaml" and profile_model == "implicit":
            actuators.append(IdealPdActuatorCfg(**common, effort_limit=effort_limit))
            actuator_types.append("IdealPD")
        else:
            actuators.append(
                DcMotorActuatorCfg(
                    **common,
                    effort_limit=effort_limit,
                    saturation_effort=saturation_efforts[i],
                    velocity_limit=motor_velocity_limits[i],
                )
            )
            actuator_types.append("DcMotor")
    ...
    print(
        ...
        f"saturation_effort={saturation_efforts}, motor_velocity_limit={motor_velocity_limits}, "
        f"delay_lag={list(zip(delay_min_lags, delay_max_lags))}, profiles={actuator_params['profile']}, "
        f"actuator_types={actuator_types}, "
...
class HumanoidVerseMjlabCore:
    def __init__(self, hv_config, mjlab_env, *, creation_config: "HumanoidVerseMjlabConfig") -> None:
        ...
        self.locomotion_mode = bool(getattr(creation_config, "locomotion_mode", False))
        ...
        for pattern in _to_list(hv_config.robot.get("terminate_after_contacts_on", [])):
            termination_contacts.extend([i for i, name in enumerate(self.body_names) if pattern in name])
        self.termination_contact_indices = torch.tensor(sorted(set(termination_contacts)), device=self.device, dtype=torch.long)
        ...
        if self.locomotion_mode:
            ...
            self.motion_dt = torch.tensor(self.dt, device=self.device)
    ...
    def _compute_reference_and_privileged_obs(self) -> None:
        ...
        if self.locomotion_mode:
            self.ref_body_pos_extend = body_pos.detach().clone()
            self.ref_body_rot_extend = body_rot.detach().clone()
            self.ref_body_vel_extend = body_vel.detach().clone()
            self.ref_body_ang_vel_extend = body_ang_vel.detach().clone()
            self.dif_global_body_pos = torch.zeros_like(body_pos)
            self.dif_joint_angles = torch.zeros_like(self.dof_pos)
            self.dif_joint_velocities = torch.zeros_like(self.dof_vel)
            ...
            self._max_local_self = torch.cat([value for value in obs_dict.values()], dim=-1)
            return
    ...
    def _locomotion_termination(self) -> tuple[torch.Tensor, torch.Tensor]:
        if not self.locomotion_mode:
        ...
        if self.termination_contact_indices.numel() == 0:
            ...
            contact_norm = self.contact_forces[:, self.termination_contact_indices].norm(dim=-1)
            crash = torch.any(contact_norm > 1.0, dim=-1)
        fall_over = self.projected_gravity[:, :2].norm(dim=-1) >= 0.9
        return crash, fall_over
    ...
    def _capture_transition_state(self) -> dict[str, torch.Tensor]:
        ...
        return {name: getattr(self, name).detach().clone() for name in field_names}

    def _terminal_observation(self) -> dict[str, torch.Tensor]:
        history = {key: value.clone() for key, value in self.history_handler.history.items()}
        observation = self.get_observation(to_numpy=False)
        self.history_handler.history = history
    ...
    def step(self, actions: torch.Tensor):
        ...
        _, _, manager_terminated, time_outs, _ = self.mjlab_env.step(mjlab_actions)
        self._refresh_state()
        reward, aux = self._compute_reward()
        crash, fall_over = self._locomotion_termination()
        terminated = torch.logical_or(manager_terminated.bool(), torch.logical_or(crash, fall_over))
        reset = torch.logical_or(terminated, time_outs.bool())
        ...
        info = {
            ...
            "termination_fall_over": fall_over,
        }
        if torch.any(reset):
            info["terminal_state"] = self._capture_transition_state()
    ...
    def reset_idx(self, env_ids: torch.Tensor, target_states: dict[str, torch.Tensor] | None = None) -> None:
        ...
        elif self.locomotion_mode:
            init_state = self.config.robot.init_state
            root_pos = torch.tensor(_to_list(init_state.pos), device=self.device, dtype=torch.float32).repeat(len(env_ids), 1)
            root_rot = torch.tensor(_to_list(init_state.rot), device=self.device, dtype=torch.float32).repeat(len(env_ids), 1)
            root_velocity = torch.cat(
                (
                    torch.tensor(_to_list(init_state.lin_vel), device=self.device, dtype=torch.float32),
                    torch.tensor(_to_list(init_state.ang_vel), device=self.device, dtype=torch.float32),
                )
            ).repeat(len(env_ids), 1)
            root_xyzw = torch.cat((root_pos, root_rot, root_velocity), dim=-1)
...
class HumanoidVerseMjlabVectorEnv(VectorEnv):
    ...
    def step(self, actions: Union[torch.Tensor, np.ndarray, Dict], to_numpy: bool = True):
        ...
        terminated = new_info.get("terminated", torch.logical_and(reset.bool(), ~time_outs)).bool()
        truncated = time_outs
        observation = self.base_env.get_observation(
            to_numpy=to_numpy,
            include_last_action=self.include_last_action,
        ...
        if to_numpy:
            ...
            for key in ("termination_crash", "termination_fall_over", "terminated"):
                if key in new_info:
                    new_info[key] = new_info[key].detach().cpu().numpy()
            for key in ("terminal_state", "terminal_observation"):
                if key in new_info:
                    new_info[key] = tree_map(lambda x: x.detach().cpu().numpy(), new_info[key])
