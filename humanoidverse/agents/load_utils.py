...
def load_model_from_checkpoint_dir(checkpoint_dir: str, device: str = "cpu", strict: bool = True) -> Any:
    checkpoint_dir = Path(checkpoint_dir)
    ...
    model = MODEL_NAME_TO_CLASS[model_name].load(checkpoint_dir / "model", device=device, strict=strict)
    return model
