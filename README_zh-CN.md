...
训练会持续把指标写入 `<work-dir>/train_log.txt`。无需中断训练，可以在另一个终端启动实时仪表盘：

```bash
uv run python -m humanoidverse.monitor runs/ufo_fb_g1
```

也可以使用仓库根目录的快捷脚本：

```bash
./monitor_cmds runs/ufo_fb_g1
```

仪表盘显示全局进度、ETA、FPS、优化器步数、checkpoint、loss/reward 趋势、机器人状态和 GPU 使用情况。省略运行目录时会自动选择 `runs/` 下最近更新的任务：

```bash
./monitor_cmds
```

脚本会把额外参数直接传给 `humanoidverse.monitor`。例如，`./monitor_cmds --once` 只输出一次，`./monitor_cmds --no-gpu` 关闭 `nvidia-smi` 查询，`./monitor_cmds --refresh 5` 将刷新间隔改为 5 秒。按 `Ctrl-C` 退出监控不会影响训练进程。
...
如果训练在远端 `piplus-soccer` 上运行，可只同步仿真需要的模型（不下载约 35 GB replay buffer 和 optimizer）：

```bash
./eval_cmds
```

`eval_cmds` 会先调用 checkpoint 同步脚本，再运行无随机化的完整 tracking inference 并保存 MP4。通过 `SYNC_CHECKPOINT=0 ./eval_cmds` 可复用本地 checkpoint，通过 `MOTION_LIST="0 3 8" ./eval_cmds` 可指定多个动作。同步脚本会在传输前后核对远端 checkpoint；如果训练恰好正在保存，会自动重试，完整下载后才切换本地 `checkpoint`。

RTX 50 系列需要包含 `sm_120` 的 CUDA 12.8 或更新 PyTorch wheel。若 `torch.cuda.get_arch_list()` 不含 `sm_120`，可在当前环境中安装同版本的 CUDA 12.8 构建；随后使用上面的 `--no-sync`，避免 `uv run` 按现有 lockfile 换回 CUDA 12.6 构建：
