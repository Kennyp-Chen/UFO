...
For identified MJLab motors, add `training.actuator.profiles` and reference a
profile from each joint. A profile with `model: ht_motor` uses the identified
motoring curve `|omega| = a*|torque|^2 + b*|torque| + c` and peak-torque
braking limit; `curve_param_a/b/c`, `max_torque`, and `max_velocity` are
required by the runtime actuator. `model: implicit` keeps a pure PD actuator.
Robots without profiles retain the legacy DC-motor path.
