...
### 6. Render an expert motion to MP4

Use the robot-aware renderer to inspect a trajectory without loading a policy checkpoint. The manifest supplies the full inference trajectory and is checked against the explicitly selected robot config:
...
./render_motion.sh \
  ...
  --motion-index 0 \
  --output renders/piplus_h1w.mp4

./render_motion.sh \
  --robot-config configs/robots/piplus_h0w.yaml \
  --data-manifest configs/data/piplus_h0w_lafan.yaml \
  --dataset piplus_h0w_lafan \
  --motion-index 0 \
  --output renders/piplus_h0w.mp4
```

Run with `--list-motions` to print motion indices, keys, frame counts, and durations. A raw UFO pkl can be selected with `--data-path`, in which case `--robot-config` is required. Use `--start-frame`, `--max-frames`, and `--stride` for short previews.
...
- [PiPlus Soccer BFM Plan](docs/piplus_soccer_bfm_plan_zh.html): object-state schema, soccer trajectory pipeline, staged implementation, and handoff notes (Chinese).
