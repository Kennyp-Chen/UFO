...
def _write_robot_state_pkl(path: Path, *, frames: int = 4, swapped_joint_names: bool = True) -> None:
    idx = np.arange(frames, dtype=np.float32)
    if swapped_joint_names:
        joint_pos = np.stack([idx + 1.0, 0.1 * (idx + 1.0)], axis=1).astype(np.float32)
        joint_names = np.asarray(["joint2", "joint1"])
    else:
        joint_pos = np.stack([0.1 * (idx + 1.0), idx + 1.0], axis=1).astype(np.float32)
        joint_names = np.asarray(["joint1", "joint2"])
    joblib.dump(
        {
            "framerate": 50,
            "base_pos_w": np.zeros((frames, 3), dtype=np.float32),
            "base_quat_w": np.tile(np.asarray([[1.0, 0.0, 0.0, 0.0]], dtype=np.float32), (frames, 1)),
            "joint_pos": joint_pos,
            "joint_names": joint_names,
        },
        path,
...
class MotionDataAdapterTest(unittest.TestCase):
    ...
    def test_robot_state_pkl_reader_reorders_wxyz_quaternion_and_joint_names(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            _, robot_config = _write_tiny_robot(root)
            spec = load_robot_spec(robot_config)
            path = root / "state.pkl"
            _write_robot_state_pkl(path, swapped_joint_names=True)
            data = read_robot_state_pkl(
                str(path),
                source_name="robot_pkl",
                robot_spec=spec,
                columns={"root_quat_order": "wxyz"},
            )
            motion = data["state"]
            np.testing.assert_allclose(motion.root_quat[0], [0.0, 0.0, 0.0, 1.0])
            np.testing.assert_allclose(motion.dof_pos[1], [0.2, 2.0])
            self.assertEqual(motion.joint_names, ["joint1", "joint2"])

    def test_robot_state_pkl_adapter_outputs_ufo_motion_dict(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            _, robot_config = _write_tiny_robot(root)
            spec = load_robot_spec(robot_config)
            path = root / "state.pkl"
            _write_robot_state_pkl(path)
            data = load_robot_state_pkl(
                str(path),
                source_name="robot_pkl",
                robot_spec=spec,
                columns={"root_quat_order": "wxyz"},
            )
            self.assertEqual(data["state"]["pose_aa"].shape, (4, 3, 3))
            np.testing.assert_allclose(data["state"]["root_quat"][0], [0.0, 0.0, 0.0, 1.0])
