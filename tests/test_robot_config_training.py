...
from humanoidverse.agents.envs.humanoidverse_mjlab import (
    ...
    _actuator_params_from_training,
    _g1_mjlab_mode15_actuator_params,
)
...
class RobotConfigTrainingTest(unittest.TestCase):
    def test_tracking_progress_includes_rate_and_eta(self) -> None:
        progress = _format_rollout_progress(
            motion_id=3,
            completed_steps=250,
            total_steps=1000,
            rendered_frames=50,
        ...
        self.assertEqual(
            progress,
            "[INFO] motion_id=3 progress=250/1000 (25.0%) rendered_frames=50 "
            "elapsed=00:00:25 rate=10.0 steps/s eta=00:01:15",
        )

    def test_tracking_z_uses_model_tracking_window(self) -> None:
        class TrackingModel:
            def __init__(self) -> None:
                self.called_with = None

            def tracking_inference(self, obs):
                self.called_with = obs
                return torch.tensor([[1.0, 2.0]])

        model = TrackingModel()
        obs = {"state": torch.zeros(1, 2)}

        z = _tracking_z(model, obs)

        self.assertIs(model.called_with, obs)
        torch.testing.assert_close(z, torch.tensor([[1.0, 2.0]]))

    def test_tracking_z_fallback_averages_future_sequence(self) -> None:
        class LegacyTrackingModel:
            cfg = SimpleNamespace(seq_length=3)
        ...
        obs = torch.tensor([[0.0], [3.0], [6.0], [9.0]])

        z = _tracking_z(LegacyTrackingModel(), obs)

        torch.testing.assert_close(z, torch.tensor([[3.0], [6.0], [7.5], [9.0]]))
    ...
    def test_piplus_h0w_profiles_select_identified_motor_models(self) -> None:
        spec = load_robot_training_spec(Path("configs/robots/piplus_h0w.yaml"))
        source, params = _actuator_params_from_training(spec.robot.control_joint_names, spec.to_env_dict())

        self.assertEqual(source, "yaml")
        self.assertEqual(params["model"].count("ht_motor"), 20)
        self.assertEqual(params["model"].count("implicit"), 2)
        self.assertEqual(params["max_torque"][10], 23.7)
        self.assertEqual(params["max_velocity"][10], 7.95)
        self.assertEqual(params["curve_param_a"][10], -0.006667)
        self.assertEqual(params["curve_param_b"][0], -0.699618)
        self.assertEqual(params["curve_param_c"][0], 19.833274)

    def test_g1_mode15_defaults_cover_generic_actuator_fields(self) -> None:
        dof_names = ("left_hip_pitch_joint", "left_wrist_yaw_joint")
        params = _g1_mjlab_mode15_actuator_params(dof_names)
        ...
        for key in (
            ...
            self.assertEqual(len(params[key]), len(dof_names))
        self.assertEqual(params["saturation_effort"], params["effort_limit"])
        self.assertEqual(params["motor_velocity_limit"], params["velocity_limit"])
        self.assertEqual(params["delay_min_lag"], [0, 0])
        self.assertEqual(params["delay_max_lag"], [0, 0])
